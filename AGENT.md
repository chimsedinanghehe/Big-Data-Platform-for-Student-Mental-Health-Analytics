...
gs://student-mental-health-lake-nhom1-2026/
  ...
  gold/
    rag_chunks/
      ...
      jsonl/
  vector/
    embeddings/student_mental_health_v1/embeddings.jsonl
  vector_backup/
...
3. Run `scripts/embeddings/index_gold_rag_chunks_to_qdrant.py --source-path=<exact_gcs_file> --merge-embedding-output` to update `embeddings.jsonl`.
4. Run the same indexing command with `--source-path=<exact_gcs_file> --no-save-embeddings --upsert-to-qdrant` to publish only that file to Qdrant.
